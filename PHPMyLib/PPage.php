...
* @version $Id: PPage.php,v 1.67 2007/01/31 09:46:27 erik Exp $
*
*/

// for backward compatibility: include PComponent if older PHPMyLib version (only php4)
if ( !defined('PML_INCLUDED') ) {

    if ( !isset($PHPMyLib_path) ) $PHPMyLib_path = 'PHPMyLib/';
    if ( @is_readable($PHPMyLib_path.'PComponent.php') ) require_once($PHPMyLib_path.'PComponent.php');
        else  require_once('PHPMyLib/PComponent.php');
...
class PTemplate extends PComponent {
    ...
    *      .
    *      .
    * <!-- END_PRINTER_FRIENDLY_COPY --></blockquote></blockquote> gekennzeichnete Bloecke werden ausgegeben )
    **/
    function PTemplate( $code = '', $file = '', $print = 0, $oldinclude = FALSE ) {
        ...
        if ( $file != '' ) {
            if ( !@is_readable($file) ) {
                ...
                $filenotfound = 1;
                foreach ($spelling as $dir) {
                    $tmp_filename = preg_replace( "/templates\//i", $dir.'/', $file );
    ...
    /**
    * Diese Methode entfernt aus dem Template saemtliche "../" aus Pfadangaben. Dreamweaver erzeugt
    ...
    **/
    function removeDir($pattern = '', $replace = '' ) {
        ...
        } else $this->m_strText = preg_replace("/(((src)|(href)|(background)|(action))=['\"]?)\.\.\//is", "\\1/", $this->m_strText);
        //$this->m_strText=str_replace('../', '', $this->m_strText);
    }
    // removeDir


    function addSelectOptions($blockname, $options = array(), $selected = array()) {
        ...
        // for compatibility with multiple selects
        if ( !is_array($selected) ) $selected = array($selected);
        foreach ($selected as $key => $value) $selected[$key] = "$value";

        $optionBlock = $selectBlock->extractBlock($blockname.'_OPTIONS');

        // remove existing value parameter and if undefined class paramater
        ...
        foreach ( $options as $entry ) {
            ...
            if ( in_array("$entry[0]", $selected) ) $optionTxt = preg_replace("/(<option[^>]*)>/is", "\\1 selected=\"selected\">", $optionTxt);
            // allow additional parameter for option line
            foreach ( $entry as $key => $value)
                    $optionTxt = str_replace('{'.$key.'}', $value, $optionTxt);
   ...
   function parse() {
        ...
        if ( preg_match_all("/\{\[(.*?)\]\}/is", $this->m_strText, $matches) ) {
            foreach ( $matches[1] as $key=>$value ) {
                $script = $value;
                // update (R.Kropp, 08.07.2004): include per webabfrage moeglich
                if ( $this->oldinclude === TRUE ) {
                    if ( FALSE && substr($script,0,1) != '.' && strtolower(substr($script,0,7)) != 'http://' ) {
                    ...
                    ob_start();
                        print_r($_SERVER);
                        print_r($_REQUEST);
                        //mail('pml@ralph.i-fabrik.net', 'old include: '.$_SERVER['HTTP_HOST'], ob_get_contents());
                    ob_end_clean();
                } else {
                    $tmp = $this->include_virtual($script);
                    // debug:
                    ob_start();
                        print_r($_SERVER);
                        print_r($_REQUEST);
                        //mail('pml@ralph.i-fabrik.net', 'include: '.$_SERVER['HTTP_HOST'], ob_get_contents());
                    ob_end_clean();
                }
   ...
   * und gibt TRUE oder FALSE zurueck, je nach Ergebnis der Suche
   * <b>Parameter:</b>
   ...
   * $name - Name des editierbaren Bereiches
   * <b>Rueckgabe:</b>
   * Template, das den editierbaren Bereich als Quelltext enthaelt
   **/
   function extractBlock($name) {
        ...
        foreach ($this->m_aVars as $varname => $varcont)
            if ( @array_key_exists($varname, $tmplBlock->m_aVars) ) $tmplBlock->m_aVars[$varname] = $varcont;
    ...
    /**
    * Diese Methode fuegt eine Komponente an die Stelle eines Platzhalters ein. Es koennen auch
    * mehrere Komponenten pro Platzhalter sein.
    * <b>Parameter:</b>
    * $var - Name des Platzhalters
    * $comp - Die einzufuegende Komponente
    **/
    function addComponent($var, $comp) {
        if ( isset($this->m_aVars[$var]) && PContainer::isPContainer($this->m_aVars[$var]) ) {
            return $this->m_aVars[$var]->add($comp);
        } else return -1;
    } //addComponent

    /**
    * ueberladene Methode von PComponent. Gibt das Template samt aller ersetzten Platzhalter aus.
    * <b>Parameter:</b>
    * keine
    * <b>Rueckgabe:</b>
    ...
    **/
    function outputStr($tpl_print=0) {
        ...
        if ( is_array($this->m_aVars) )
            foreach ( $this->m_aVars as $varname=>$varcont )
                $text = str_replace("{".$varname."}", $varcont->outputStr(), $text);
...
/**
* Die Klasse PTemplate2 verwaltet Templates (aehnlich Dreamweaver) und ist in der Lage
* Ersetzungen von Platzhaltern der Form {name} und editierbaren Bereichen der Form
* <!-- BeginEditable2 "name"> ... <!-- EndEditable2 "name"> durchzufuehren
...
/**
* Die Klasse PTemplate3 verwaltet Templates (aehnlich Dreamweaver) und ist in der Lage
* Ersetzungen von Platzhaltern der Form {name} und editierbaren Bereichen der Form
* <!-- BeginEditable "name"> ... <!-- EndEditable "name"> durchzufuehren
