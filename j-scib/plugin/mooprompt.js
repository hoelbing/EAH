...
var MooPrompt = box = new Class({
	setOptions: function(options){
		this.options = {
			...
			effect     : 'grow', // 'grow' - Expands box from a middle point and fades in content || 'slide' - Slides in the box from the nearest side.
			opacity    : 0.6,
			duration   : 400
	...
	initialize: function(title, content, options){
		...
		if( this.options.overlay ){
			if( $('cbOverlay') ){
				if( this.options.opacity )
					$('cbOverlay').setOpacity(this.options.opacity);
		...
		for (var i = 1; i <= this.options.buttons; i++) {
			...
			this['button'+i].addClass(this.options['buttonClass'+i]);
		}
		...
		if( this.options.effect=='grow' ){
			...
			if( this.options.vertical=='top' ){
				...
				if( this.options.vertical == 'bottom' )
			...
			this.fx2 = new Fx.Styles(this.container, {duration: this.options.duration});
			this.fx2.custom({
				'width'        : [0, this.options.width],
				'margin-left'  : [0, -this.options.width/2],
				'margin-right' : [0, -this.options.width/2],
				'height'       : [0, this.boxHeight],
				'margin-top'   : [0, -this.boxHeight/2],
				'margin-bottom': [0, -this.boxHeight/2]
			}).chain(function(){
				this.box.setStyles({
					'visibility': 'hidden',
					'display'   : '',
				...
				new Fx.Style(this.box, 'opacity', {duration: this.options.duration}).custom(0, 1).chain(function(){
	...
	close: function(fn) {
		...
		new Fx.Style(this.container, 'opacity', {duration:this.options.duration,
			onComplete: function() {
				window.removeEvent('scroll', this.eventPosition).removeEvent('resize', this.eventPosition);
				this.container.remove();
				if( this.iframe ) this.iframe.remove();
				if( this.options.overlay ){
					/*new Fx.Style(this.overlay, 'opacity', {duration:this.options.transitions,
					onComplete: function(){
						this.overlay.remove();
					}.bind(this)
					}).custom(this.options.opacity, 0);*/
					this.overlay.remove();
				}
			}.bind(this)
		}).custom(1, 0);
